***DC-OPF integrated with Energy Storage and Wind
...
Table GD(Gen,*) 'generating units characteristics'
     Pmax Pmin    Qmax Qmin  b      CostsD  costst  RU  RD   SU   SD   UT  DT  uini  U0  S0
g1   400  100     300  -100  5.47   0       0       47  47   105  108  1   1   1     5   0
g2   400  100     310  -100  5.47   0       0       47  47   106  112  1   1   1     6   0
g3   152  30.4    115  -40   13.32  1430.4  1430.4  14  14   43   45   8   4   1     2   0
g4   152  30.4    115  -40   13.32  1430.4  1430.4  14  14   44   57   8   4   1     2   0
g5   155  54.25   121  -45   16     0       0       21  21   65   77   8   8   0     0   2
g6   155  54.25   115  -45   10.52  312     312     21  21   66   73   8   8   1     10  0
g7   310  108.5   230  -75   10.52  624     624     21  21   112  125  8   8   1     10  0
g8   350  140     250  -80   10.89  2298    2298    28  28   154  162  8   8   1     5   0
g9   350  75      250  -80   20.7   1725    1725    49  49   77   80   8   8   0     0   2
g10  591  206.85  320  -140  20.93  3056.7  3056.7  21  21   213  228  12  10  0     0   8
g11  60   12      40   -10   26.11  437     437     7   7    19   31   4   2   0     0   1
g12  300  0       210  -74   0      0       0       35  35   315  326  0   0   1     2   0;
...
Parameter Wcap(bus)/8 200,19 150,21 100/, SOCMax(bus)/19 200,21 100/;
Scalar eta_c/0.95/, eta_d/0.9/, VWC/50/;
Parameter SOC0(bus); SOC0(bus)= 0.2*SOCMax(bus)/sbase;
branch(bus,node,'r')$(branch(bus,node,'r')=0)= branch(node,bus,'r');
...
branch(node,bus,'theta')$branch( node,bus,'limit')= arctan(branch(node,bus,'x')/branch(node,bus,'r'));

Parameter conex(bus,node);
conex(bus,node)$(branch(bus,node,'limit')and branch(node,bus,'limit'))= 1;
conex(bus,node)$(conex(node,bus))= 1;

Variable OF, Pij(bus,node,t), Qij(bus,node,t), Pg(Gen,t), Qg(Gen,t), delta(bus,t), V(bus,t), lsh(bus,t), Pw(bus,t), pwc(bus,t), SOC(bus,t), Pd(bus,t), Pc(bus,t);
Equation const1, const2, const2B, const3, const4, const5, const6, const7, const8, constESS;
const1(bus,node,t)$(conex(bus,node)).. Pij(bus,node,t) =e= branch(bus,node,'bij')*(delta(bus,t)-delta(node,t));
const2(bus,t).. lsh(bus,t)$BusData(bus,'pd')+Pw(bus,t)$Wcap(bus)+sum(Gen$GB(bus,Gen),Pg(Gen,t))+(-Pc(bus,t)+Pd(bus,t))$SOCMAX(bus)-WD(t,'d')*BusData(bus,'pd')/Sbase =e= sum(node$conex(node,bus),Pij(bus,node,t));
const2B(bus,t).. sum(Gen$GB(bus,Gen),Qg(Gen,t))-WD(t,'d')*BusData(bus,'Qd')/Sbase =e= sum(node$conex(node,bus),Qij(bus,node,t));
const3.. OF =g= sum((bus,Gen,t)$GB(bus,Gen), Pg(Gen,t)*GD(Gen,'b')*Sbase)+sum((bus,t),VOLL*lsh(bus,t)*Sbase$BusData(bus,'pd')+VOLW*Pwc(bus,t)*sbase$Wcap(bus));
...
const7(bus,node,t)$conex(bus,node).. Pij(bus,node,t) =e= V(bus,t)*V(bus,t)*cos(branch(node,bus,'theta'))/branch(node,bus,'z')-V(bus,t)*V(node,t)*cos(delta(bus,t)-delta(node,t)+branch(node,bus,'theta'))/branch(node,bus,'z');
const8(bus,node,t)$conex(bus,node).. Qij(bus,node,t) =e= V(bus,t)*V(bus,t)*sin(branch(node,bus,'theta'))/branch(node,bus,'z')-V(bus,t)*V(node,t)*sin(delta(bus,t)-delta(node,t)+branch(node,bus,'theta'))/branch(node,bus,'z')-branch(node,bus,'b')*V(bus,t)*V(bus,t)/2;

Pg.lo(Gen,t)= GD(Gen,'Pmin')/Sbase; Pg.up(Gen,t)= GD(Gen,'Pmax')/Sbase;
Qg.lo(Gen,t)= GD(Gen,'Qmin')/Sbase; Qg.up(Gen,t)= GD(Gen,'Qmax')/Sbase;
delta.up(bus,t)= pi/2; delta.lo(bus,t)= -pi/2; delta.fx(slack,t)= 0;
V.up(bus,t)=1.05; V.lo(bus,t)=0.95; V.l(bus,t)=1;
Qij.up(bus,node,t)$((conex(bus,node)))= branch(bus,node,'Limit')/Sbase;
Qij.lo(bus,node,t)$((conex(bus,node)))= -branch(bus,node,'Limit')/Sbase;
...
Pwc.up(bus,t)= WD(t,'w')*Wcap(bus)/Sbase; Pwc.lo(bus,t)= 0;

SOC.up(bus,t)= SOCmax(bus)/sbase; SOC.lo(bus,t)= 0*SOCmax(bus)/sbase; SOC.fx(bus,'t24')= SOC0(bus);
Pc.up(bus,t) = 0.2*SOCmax(bus)/sbase; Pc.lo(bus,t) = 0;
Pd.up(bus,t) = 0.2*SOCmax(bus)/sbase; Pd.lo(bus,t) = 0;
Model loadflow /all/;
...
display report, Pij.l, Qij.l, Pg.l, Qg.l, Congestioncost;
$onText
...
*$ifThen not errorLevel 1
   ...
   execute_unload "opf.gdx" Pwc.l
   execute 'gdxxrw.exe  OPF.gdx o=OPF.xls var=Pwc rng=PC!A1'
   ...
   execute_unload "opf.gdx" report2
   execute 'gdxxrw.exe  OPF.gdx o=OPF.xls par=report2 rng=report2!A1'
*$endIf
$offText
