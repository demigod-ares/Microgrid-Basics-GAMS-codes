...
Parameter Bmatrix(bus,node),Binv(bus,node);
...
parameter inva(nonslack,nonslackj) 'inverse of a', Dfactor(bus,node,knot,shin), contingency(bus,node,knot,shin);
...
dfactor(bus,node,knot,shin)$(conex(bus,node)and conex(knot,shin)and(ord(bus)>ord(node))and(ord(knot)>ord(shin))and(ord(bus)<>ord(knot)or ord(node)<>ord(shin)))=branch(bus,node,'bij')*(Binv(bus,knot)-Binv(bus,shin)-Binv(node,knot)+Binv(node,shin))/(1-branch(knot,shin,'bij')*(Binv(knot,knot)+Binv(shin,shin)-2*Binv(knot,shin)));
Display Bmatrix, Breduced, Binv, GSHF, dfactor;
