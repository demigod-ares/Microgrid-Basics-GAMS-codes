*** Transmission Expansion Planning (2 years period & 10 years period)

Set bus/1*6/, slack(bus)/1/, Gen/g1*g3/, k/k1*k4/;
Scalar Sbase/100/, M/1000/;
Alias (bus,node);
...
Variable OF, OPC, INVC, Pij(bus,node,k), Pg(Gen), delta(bus), LS(bus);
...
Equation const1A, const1B, const1C, const1D, const1E, const2, const3, equ1, equ2;
...
OF =g= 10*8760*OPC + INVC;
equ1..
   OPC =e= sum(Gen, Pg(Gen)*GenData(Gen,'b')*Sbase)+100000*sum(bus ,LS(bus));
equ2..
   INVC =e= 1e6*sum((bus,node,k)$conex(node,bus),0.5*branch(bus,node,'cost')*alpha(bus,node,k)$(ord(k)>1 or branch(node,bus,'stat')=0));
